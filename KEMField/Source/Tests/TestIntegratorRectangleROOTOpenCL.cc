...
int main()
{
	...
	for( unsigned int k=0; k<=kmax; k++ ) {
		...
		for( it=container->begin<KElectrostaticBasis>(); it!=container->end<KElectrostaticBasis>(); ++it ) {
            ...
            valAna = std::make_pair(intOCLAna.ElectricField(itRect->GetShape(),fP),intOCLAna.Potential(itRect->GetShape(),fP));
            valNum = std::make_pair(intOCLNum.ElectricField(itRect->GetShape(),fP),intOCLNum.Potential(itRect->GetShape(),fP));
            valRwg = std::make_pair(intOCLRwg.ElectricField(itRect->GetShape(),fP),intOCLRwg.Potential(itRect->GetShape(),fP));
#else

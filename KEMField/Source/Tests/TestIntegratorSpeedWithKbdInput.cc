...
double totalTime=0.;
    ...
    StartTimer();
#ifdef KEMFIELD_USE_OPENCL
        fOpenCLIntegratingFieldSolver->ElectricFieldAndPotential( KEMThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
        //fOpenCLIntegratingFieldSolver->Potential( KEMThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
#else
        fIntegratingFieldSolver->ElectricFieldAndPotentialNoKahanSum( KEMThreeVector(fieldPoints[(i*3)],fieldPoints[(i*3)+1],fieldPoints[(i*3)+2]) );
        totalTime+=Time();
#endif
    }
    KEMField::cout << "The computation of electric field and potential simultaneously on " << noPoints << " points took " << totalTime << " seconds." << KEMField::endl;

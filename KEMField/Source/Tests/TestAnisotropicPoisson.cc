...
{
public:
  KIdentity(unsigned int dimension) :
  KSquareMatrix<double>(),
  fDimension(dimension),
  fOne(1.),
  fZero(0.) {}
...
public:
  ...
  unsigned int Dimension() const { return fA.Dimension()*fB.Dimension(); }
...
public:
  ...
  unsigned int Dimension() const { return fN_x*fN_z; }
...
//      break;
  ...
  {
    KSuccessiveSubspaceCorrection<double,

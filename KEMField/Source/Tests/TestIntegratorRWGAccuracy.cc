...
void AddTriangle( std::vector<KEMTriangle> &v, KEMThreeVector triP0, KEMThreeVector triP1, KEMThreeVector triP2 )
{
	KEMTriangle newTri;
    ...
    newTri.SetN1( KEMThreeVector((triP1[0]-triP0[0]) / newTri.GetA(),
                                 (triP1[1]-triP0[1]) / newTri.GetA(),
                                 (triP1[2]-triP0[2]) / newTri.GetA()) );

    newTri.SetN2( KEMThreeVector((triP2[0]-triP0[0]) / newTri.GetB(),
...
int main()
{
    ...
    for( unsigned int i=0; i<elementNo; i++ )
    {
        ...
        if( print ) std::cout << "P0 = ";
        for( unsigned short l=0; l<3; l++ ) {
        	triP0[l]=-1.+(2.*randomnumber());
        	if( print ) std::cout << triP0[l] << "  ";
        }
        if( print ) std::cout << "  P1 = ";
        for( unsigned short j=0; j<3; j++ ){
        	triP1[j]=-1.+(2.*randomnumber()); // = fP0 + fN1*fA
        	if( print ) std::cout << triP1[j] << "  ";
        }
        if( print ) std::cout << "  P2 = ";
        for( unsigned short k=0; k<3; k++ ){
        	triP2[k]=-1.+(2.*randomnumber()); // = fP0 + fN2*fB
        	if( print ) std::cout << triP2[k] << "  ";
        }
        std::cout << std::endl;
...
// THIS IS NEVER USED
        ...
        triangle->SetP0( KEMThreeVector(triP0[0],triP0[1],triP0[2]) );
        triangle->SetN1( KEMThreeVector(triData[5],triData[6],triData[7]) );
        triangle->SetN2( KEMThreeVector(triData[8],triData[9],triData[10]) );
        ...
        KEMThreeVector fP(0., 0., 0.);
        fP = KEMThreeVector(triData[2], triData[3], triData[4])
                + (dirN1*KEMThreeVector(triData[5],triData[6],triData[7]))
                + (dirN2*KEMThreeVector(triData[8],triData[9],triData[10]) );

        if( print ) std::cout << "fP = " << fP[0] << "  " << fP[1] << "  " << fP[2] << std::endl;

        KEMThreeVector fPN3(0., 0., 0.);
       ...
       // FOR WHAT IF IT IS NEVER USED?
        ...
        if( print ) std::cout << distToLine << "   ";
        if( print ) std::cout << lineLambda << "   ";
        ...
        if( print ) std::cout << "final min distance " << distToLineMin << " with index " << lineIndex << std::endl;
        ...
        KEMThreeVector goOut;

        // decide which m has to be taken
        if( lineIndex==0 ) goOut = KEMThreeVector( m0[0], m0[1], m0[2] ); // Unit vectors ???????
        if( lineIndex==1 ) goOut = KEMThreeVector( m1[0], m1[1], m1[2] );
        if( lineIndex==2 ) goOut = KEMThreeVector( m2[0], m2[1], m2[2] );

        KEMThreeVector goAlong;

        if( lineIndex==0 ) goAlong = KEMThreeVector( triAlongSideP0P1Unit[0], triAlongSideP0P1Unit[1], triAlongSideP0P1Unit[2] );
        if( lineIndex==1 ) goAlong = KEMThreeVector( triAlongSideP1P2Unit[0], triAlongSideP1P2Unit[1], triAlongSideP1P2Unit[2] );
        if( lineIndex==2 ) goAlong = KEMThreeVector( triAlongSideP2P0Unit[0], triAlongSideP2P0Unit[1], triAlongSideP2P0Unit[2] );

        // save rectangle points

        std::vector<KEMThreeVector> rect;
        ...
        KEMThreeVector p0Rel;
        KEMThreeVector p1Rel;
        KEMThreeVector p2Rel;

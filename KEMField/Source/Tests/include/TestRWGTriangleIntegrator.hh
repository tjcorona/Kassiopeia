...
void IqLFieldAndPotential( double* data, const double* P,
		const unsigned short countCross, const unsigned short lineIndex, const double dist,
		double* result )
{
    // corner points P0, P1 and P2
	...
	for( unsigned short i=0; i<3; i++ ) result[i] = iLField[i];
	result[4] = iLPhi;
}
...
void FieldPotTriRWG( double* triData, const double* P, double* result )
{
    ...
    if( correctionCounter == 2 ) {
    	const double upEps[3] = {
    			P[0] + M_DISTANCECORRECTIONN3*triN3[0],
    	...
    	result[0] = M_ONEOVER_4PI_EPS0 * 0.5 * ((triN3[0]*solidAngleUp + IqLFieldAndPotentialUp[0]) + (triN3[0]*solidAngleDown + IqLFieldAndPotentialDown[0]));
		result[1] = M_ONEOVER_4PI_EPS0 * 0.5 * ((triN3[1]*solidAngleUp + IqLFieldAndPotentialUp[1]) + (triN3[1]*solidAngleDown + IqLFieldAndPotentialDown[1]));
		result[2] = M_ONEOVER_4PI_EPS0 * 0.5 * ((triN3[2]*solidAngleUp + IqLFieldAndPotentialUp[2]) + (triN3[2]*solidAngleDown + IqLFieldAndPotentialDown[2]));

    	result[3] = M_ONEOVER_4PI_EPS0 * 0.5 * ((-hUp*solidAngleUp - IqLFieldAndPotentialUp[3]) + (-hDown*solidAngleDown - IqLFieldAndPotentialDown[3]) );

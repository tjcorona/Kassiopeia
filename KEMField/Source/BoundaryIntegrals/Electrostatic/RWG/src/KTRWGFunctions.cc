...
void KTRWGFunctions::InitVectors()
{
	...
	double n[3];
	for(unsigned int k = 0; k < 3; k++) n[k] = fElectrode -> GetNormal(P,k);
	fNormal = n;
	...
	for(unsigned int i = 0; i < fNVerticies; i++) fOrder.at(i) = i;

	if (fNVerticies == 3) {

		TVector3 b = ((tmpVector.at(1) - tmpVector.at(0)).Cross((tmpVector.at(2) - tmpVector.at(0)))).Unit();

		if (b.Dot(fNormal) < 0.) {fOrder.at(0)=0;fOrder.at(1)=2;fOrder.at(2)=1;}
	}

	fVertex.reserve(fNVerticies);
	for(unsigned int i = 0; i < fNVerticies; i++) fVertex.push_back(tmpVector.at(fOrder.at(i)));
	...
	for(unsigned int i = 0; i < fNVerticies; i++) a_n.push_back((fVertex.at(i) - fPosition).Unit());
...
double KTRWGFunctions::FunctionIqS(int qIndex)
{
	...
	if (GetIqS(qIndex, aValue)) return aValue;

	int qStart = (qIndex+3)%2 - 3;
	for(int q = qStart; q <= qIndex; q+=2 )
	{
		if (q == -3) {
			if (fabs(fH) > 0.) aValue = GetSolidAngle() / fH;
			else if (fDeltaR.Mag()==0.) aValue = GetSolidAngle();
		}
		else if (q > -2)
		{
...
double KTRWGFunctions::FunctionIqL(int qIndex, unsigned int iStart, unsigned int iEnd)
{
	...
	if (GetIqL(qIndex, iStart, iEnd, aValue)) return aValue;
...
void KTRWGFunctions::CalcSolidAngle()
{
	...
	} else
	{
		...
		for(unsigned int i = 0; i < fNVerticies; i++){
			...
			if ((dalpha) < 0.) dalpha += TMath::TwoPi();
			angle_sum += dalpha;
		}
	...
	if (fDeltaR.Mag() == 0.) fSolidAngle = TMath::TwoPi();

	if (fH < 0.) fSolidAngle *= -1.;
}
...
bool KTRWGFunctions::GetIqS(int index, double& aValue) const
{
	...
	if (isValid) aValue = fIqS_Map.find(index)->second;
...
bool KTRWGFunctions::GetIqL(int index, unsigned int iStart, unsigned int iEnd, double& aValue) const
{
	...
	if (isValid) aValue = fIqL_Map.find(key)->second;
...
std::vector<std::complex <double> > KTRWGFunctions::CalcGreenFunction(const double k, const unsigned int iType)
{
	...
	int qStart = -1; int qEnd = std::min(fQmin,fQmax);
	for(int q = qStart; q < qEnd; q++)
	{
		...
		for(int l = qStart; l <= q; l++)
		{
			...
			coefficient = pow(-1.,q-l) * pow(im,q+1) * pow(k,l+1) / TMath::Factorial(l+1) / TMath::Factorial(q-l);
			...
			if (abs(coefficient) > 0.) {
				...
				case kGradient :
					...
					if (fabs(fH) > 1.e-13) KqNterm  = (iqlterm - fH * l * fNormal * FunctionIqS(l-2));
					else KqNterm = iqlterm;// - l * fNormal * FunctionIqS(l-2);
					break ;
				case kCurl :
					for(unsigned int i = 0; i < fNVerticies; i++){
						unsigned int j = (i + 1) % fNVerticies;
				...
				case kCurlScalar:
					for(unsigned int i = 0; i < fNVerticies; i++){
						unsigned int j = (i + 1) % fNVerticies;
						iqlterm += UnitVector * (fRho - fSourcePoint).Dot((fAlongSide.at(i)).Unit()) * FunctionIqL(l, i , j);
						iqlterm += UnitVector * (1./(l+2.)) * (pow((fVertex.at(i) - fPosition).Mag(),l+2) - pow((fVertex.at(j) - fPosition).Mag(),l+2));
					}
					KqNterm = iqlterm;
					break;
				...
				for(unsigned int i = 0; i<3; i++) aValue[i] += coefficient * KqNterm[i];
				if (q > -1) fPrecisionTrack += coefficient * pow(fDeltaR.Mag(),l);
			}
		}
		diff = 0.;

		if (q < 1) for(unsigned int i = 0; i<3; i++) saveFirst[i] = aValue[i];

		for (unsigned int i = 0; i<3; i++) {
			if (abs(aValue_lookback[i])>0.) diff += abs(aValue_lookback[i]- aValue[i])/abs(aValue_lookback[i]);
		}
		if (diff > fAccuracy) qEnd = TMath::Min(q+2, fQmax);
	...
	if ( ((diff > fAccuracy) && (qEnd == fQmax)) && (abs(fPrecisionTrack) > fAccuracy)) {
		for(int i = 0; i < 3; i++) aValue[i] = saveFirst[i] * exp(im * kPhase);
	}
...
double KTRWGFunctions::Potential(double* pPosition, bool /*sym*/, double k)
{
	...
	bool isEqual = true;
	for(int i = 0; i < 3; i++) if (pPosition[i] !=  fPosition[i]) isEqual = false;
...
void KTRWGFunctions::EField(double *pPosition, double *field,bool /*sym*/, double k)
{
	...
	for(int i = 0; i <3; i++) field[i] = real(qGradient[i] / KTElectrode::fEps0);
...
std::vector<std::complex <double> > KTRWGFunctions::GetVectorIntegral(){
	...
	for(int k = 0; k < 3; k++) qVectorIntegral[k] = iqlterm[k];
...
std::vector<Complex_t> KTRWGFunctions::GetSmoothIntegral(const double k, const unsigned int iType)
{
	...
	//! Combine results
	for(unsigned int j = 0; j < nDim; j++) GreenIntegral[j] = Gs[j] + real(Ge[j]);
